...
{
  uint8_t data_write = LM75_REG_TEMP;
  uint8_t data_read[2];
  int16_t raw;

  HAL_I2C_Master_Transmit(&hi2c1, LM75_ADDR, &data_write, 1, HAL_MAX_DELAY);
  HAL_I2C_Master_Receive(&hi2c1, LM75_ADDR, data_read, 2, HAL_MAX_DELAY);

  raw = (data_read[0] << 8) | data_read[1];
  return raw / 256.0f;
}
/* USER CODE END 0 */
...
int main(void)
{
  ...
  tempQueueHandle = osMessageQueueNew (60, 4, &tempQueue_attributes);
...
void StarttempTask(void *argument)
{
  ...
  /* Infinite loop */
    ...
    for (;;)
    {
        temp = LM75_ReadTemp_HAL();
        osMessageQueuePut(tempQueueHandle, &temp, 0, 0);

        if (temp > 28.0f)
        {
            osEventFlagsSet(alarmEventHandle, 0x01);
        }
        else if (temp < 26.0f)
        {
            osEventFlagsClear(alarmEventHandle, 0x01);
        }
...
void StartalarmTask(void *argument)
{
	osEventFlagsWait(alarmEventHandle, 0x01,osFlagsWaitAny, osWaitForever);
  /* USER CODE BEGIN StartalarmTask */
  /* Infinite loop */
for (;;)
{
    if (osEventFlagsGet(alarmEventHandle) & 0x01)
    {
        HAL_GPIO_TogglePin(LD1_GPIO_Port, LD1_Pin);
        osDelay(300);
    }
...
void StartserialTask(void *argument)
{
  ...
  /* Infinite loop */
   ...
   for (;;)
    {
        if (osMessageQueueGet(tempQueueHandle, &temp, NULL, 100) == osOK)
        {
            buffer[idx] = temp;
            idx = (idx + 1) % 60;
            if (count < 60) count++;
